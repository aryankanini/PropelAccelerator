...
- [ ] Define typed repositories for approval state and immutable audit revisions. (AC-001)
- [ ] Define a unit-of-work transaction boundary for related review writes. (AC-001)
- [ ] Persist approval state and audit revision before one explicit commit. (AC-001)
- [ ] Roll back all pending review changes when either write fails. (AC-001)
- [ ] Return a recoverable connection failure without an approved outcome. (edge case)